...
gulp.task( 'clean', function() {
  return del( [ 'static/js/**/*', 'static/css/**/*' ] ); //'static/fonts/**/*','static/images/**/*'
} );
gulp.task( 'jshint', function() {
  return gulp.src( options.JS_SRC.concat( '!' + options.JS_SRC[ 0 ] ) // excluding vendor libs from lint checking
    ...
    } ) ) // adding angular to global scope to avoid angular not found errors in lint
    .pipe( jshint.reporter( stylish ) );
} );
gulp.task( 'template-cache', function() {
  return gulp.src( options.PARTIALS_SRC ).pipe( htmlmin( {
    collapseWhitespace: true,
    removeComments: true
  } ) ).pipe( templateCache( options.TEMPLATES_DEST_NAME, {
    standalone: true
  } ) ).pipe( gulp.dest( options.PARTIALS_DEST ) );
} );
gulp.task( 'js', [ 'jshint', 'template-cache' ], function() {
  return gulp.src( options.JS_SRC_PROD ).pipe( ngmin() ).pipe( uglify() ).pipe( concat( 'app.min.js' ) ).pipe( gulp
    .dest( options.JS_DEST ) );
} );
gulp.task( 'js-dev', [ 'jshint', 'template-cache' ], function() {
  return gulp.src( options.JS_SRC_DEV ).pipe( concat( options.JS_DEST_NAME ) ).pipe( gulp.dest( options.JS_DEST ) );
} );
...
gulp.task( 'css', [ 'compile-sass' ], function() {
  return gulp.src( options.CSS_SRC ).pipe( concat( options.CSS_DEST_NAME ) ).pipe( autoprefixer( {
    browsers: [ 'last 2 versions' ],
    cascade: false
  } ) ).pipe( minifycss() ).pipe( gulp.dest( options.CSS_DEST ) );
} );
